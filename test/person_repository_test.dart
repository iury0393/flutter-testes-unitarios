...
void main() {
  ...
  test('deve pegar uma lista de person', () async {
    // when(repository).thenAnswer((_) async => http.Response(jsonReturn, 200));
  ...
  test('retorna uma exception se nao for statuCode 200', () async {
    // when(client).thenAnswer((_) async => http.Response(jsonReturn, 404));
    expect(() async => repository.getPerson(), throwsException);
  });
}
